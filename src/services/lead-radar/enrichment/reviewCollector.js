...
async function fetchHtml(url, {
  ...
  throw lastError || new Error('Unable to fetch Google Maps review HTML');
}

function parseReviewCount(html) {
  const patterns = [
    /"reviewCount"\s*:\s*"?(?<count>[0-9][0-9,]*)"?/i,
    /(?<count>[0-9][0-9,]*)\s+reviews/i,
    /aria-label="(?<count>[0-9][0-9,]*)\s+reviews"/i,
  ];

  for (const pattern of patterns) {
    const match = html.match(pattern);
    const value = Number(String(match?.groups?.count || '').replace(/,/g, ''));
...
export async function collectReviewCount({
  hotelId,
  googleMapsUrl,
  fetchImpl = globalThis.fetch,
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  const updated = await hotelEnrichmentRepository.upsertHotelEnrichment(
    mapExistingToUpsert(existing, {
      hotelId,
      reviewCount,
      reviewSource: googleMapsUrl,
      reviewLastCheckedAt: new Date().toISOString(),
    }),
  );
...
export async function collectReviewsForHotels(
  hotels = [],
  sourceResolver = () => ({}),
  options = {},
) {
  ...
  for (const hotel of hotels) {
    ...
    results.push(await collectReviewCount({
      hotelId: hotel?.hotelId ?? hotel?.id ?? null,
