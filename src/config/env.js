...
const env = {
  ...
  enableAugustImportantDateGenerator: parseBoolean(
    process.env.ENABLE_AUGUST_IMPORTANT_DATE_GENERATOR,
    true,
  ),
  ...
  realtimeSignalSnapshotFile: readOptionalString('REALTIME_SIGNAL_SNAPSHOT_FILE', ''),
  realtimeSignalCollectorCommand: readOptionalString('REALTIME_SIGNAL_COLLECT_COMMAND', ''),
  ...
  publicMarketLiveSourcesDir: readOptionalString(
    'PUBLIC_MARKET_LIVE_SOURCES_DIR',
    isProduction ? '/opt/radar_light/shared/live_sources' : 'shared/live_sources',
  ),
  ...
  centralIntelligenceWatchActionConfidence: parseNumber(process.env.CI_WATCH_ACTION_CONFIDENCE, 60),
  centralIntelligenceHoldActionConfidence: parseNumber(process.env.CI_HOLD_ACTION_CONFIDENCE, 40),
  centralIntelligenceCompetitorFreshHours: parseNumber(process.env.CI_COMPETITOR_FRESH_HOURS, 36),
  centralIntelligenceHotelRateFreshHours: parseNumber(process.env.CI_HOTEL_RATE_FRESH_HOURS, 24),
  ...
  emailFrom: readOptionalString('EMAIL_FROM', ''),
  emailReplyTo: readOptionalString('EMAIL_REPLY_TO', ''),
